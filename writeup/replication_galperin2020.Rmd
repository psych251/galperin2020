...
Since I did have two t-tests, I needed to take the results and multiply by 2 to get the results for power analysis for the whole study. Therefore, the final results for the power analysis were as follows:
...
The sample consists of `150` participants recruited from Prolific. In order to participate in the study, individuals should have an experience hiring people for the organization because this study addresses job decision making processes of hiring managers in the labor market. They also have to be residents of U.S. or Canada because job candidate's qualification and evaluations depend on the nuiances in the labor market, which may be different across different regions.
\
...
`I did not know where to put procedures of Pilot B so I am adding a subsection here.`
...
Therefore I conducted the second Pilot B with different sampling strategy. I added another filter so that only residents of U.S. or Canada can participate in the study. I also deleted all the open-ended questions from the survey. After running the second pilot, I could see that the problem was more about participants' country of residence than having open-ended questions in the survey.
Hence, I ran the third Pilot B with the original survey design. The sampling strategy was the same as the second pilot (individuals with hiring experience who live in the U.S. or Canada, fluent in English). The third pilot ran without any problem and I was able to collect data.
\
...
```{r, echo = TRUE}
# load survey
qualtricsData_pilotB <- fetch_survey(surveyID = "SV_d4LnrTl3Leh1WDQ", force_request = T)
```

```{r, echo = TRUE}
# save data
write_csv(qualtricsData_pilotB, "/Users/jbyun/Dropbox/2021 Fall/PSYCH 251/galperin2020/data/pilotB_full.csv")
```

```{r, echo = TRUE}
# load data from csv
pilotB_df <- read_csv("/Users/jbyun/Dropbox/2021 Fall/PSYCH 251/galperin2020/data/pilotB_full.csv")
```
...
# add id column
data_cleaning <- tibble::rowid_to_column(data_cleaning, "id")
